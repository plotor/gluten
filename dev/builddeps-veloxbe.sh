...
## install arrow
### ./get_arrow.sh --enable_custom_codec=OFF
### ./build_arrow.sh --build_type=Release --build_tests=OFF --build_benchmarks=OFF --enable_ep_cache=OFF
if [ "$SKIP_BUILD_EP" != "ON" ]; then
    cd $GLUTEN_DIR/ep/build-arrow/src
    ./get_arrow.sh --enable_custom_codec=$ARROW_ENABLE_CUSTOM_CODEC
    ./build_arrow.sh --build_type=$BUILD_TYPE \
...
## install velox
### ./get_velox.sh --enable_hdfs=ON --build_protobuf=ON --enable_s3=OFF
### ./build_velox.sh --enable_s3=OFF --build_type=Release --enable_hdfs=ON --enable_ep_cache=OFF --build_benchmarks=OFF
if [ "$SKIP_BUILD_EP" != "ON" ]; then
    cd $GLUTEN_DIR/ep/build-velox/src
    ./get_velox.sh --enable_hdfs=$ENABLE_HDFS --build_protobuf=$BUILD_PROTOBUF --enable_s3=$ENABLE_S3
    ./build_velox.sh --enable_s3=$ENABLE_S3 \
